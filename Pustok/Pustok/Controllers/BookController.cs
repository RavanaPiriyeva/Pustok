...
namespace Pustok.Controllers
{
    public class BookController : Controller
    {
        ...
        public IActionResult Detail(int id)
        {
            ...
            {
                Book = book,
                RelatedBooks = _context.Books.Include(x => x.BookImages).Include(x => x.Author).Where(x => x.CategoryId == book.CategoryId).Take(6).ToList(),
        ...
        {
            AppUser user = (AppUser)await _context.Users.FirstOrDefaultAsync(x => x.NormalizedUserName == User.Identity.Name.ToUpper());
